...
// ---------------------------------------------------------------------------
// SawPhasor: naive saw oscillator
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Echo: feedback-aware delay line
// ---------------------------------------------------------------------------
...
impl AudioIODeviceCallback for MyDsp {
    ...
    fn process_block(
        ...
        // update freq from params (random thread + TUI)
        self.saw.set_freq(load_f32(&self.params.freq));

        let volume = load_f32(&self.params.volume);
        let bypass = self.params.bypass.load(Ordering::Relaxed);

        // smoothed echo feedback
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Random frequency modulation (like your old code)
    {
    ...
    let dsp = MyDsp {
        saw: SawPhasor::new(440.0, config.sample_rate),
        echo: Echo::new(10000, 0.2),
        params: Arc::clone(&params),
        smoothed_feedback: 0.2,
    };
